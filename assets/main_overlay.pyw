...
UI.attributes("-transparentcolor", "black")  # noqa
UI.overrideredirect(True)# Remove window border


# Make the window click-through
UI.update()
mainhwnd = win32gui.FindWindow(None, str(UI.title())) # noqa
...
###############################################################################################################
try:
    main_overlay_motivation_color = ConfigManager().load_from_config(config_name="config.json", name_in_config="main_overlay_motivation_color")
    main_overlay_motivation_size = ConfigManager().load_from_config(config_name="config.json", name_in_config="main_overlay_motivation_size")
    ...
    print("Value Error")
    main_overlay_motivation_color = "rgb"
    main_overlay_motivation_size = 15
...
def update_spruch():
    try:
        motivations_spruch = random.choice(motivation)
    except IndexError:
        motivations_spruch = ""
    canvas.itemconfig(mot_id, text=motivations_spruch)
    UI.after(300000, update_spruch) # noqa
...
mot_id = canvas.create_text(
    ...
    anchor="ne",
    fill="#00ff9d",
    font=("Comic Sans MS", main_overlay_motivation_size, "bold italic")
)
update_spruch()

if main_overlay_motivation_color == "rgb":
    update_color()
else:
    canvas.itemconfig(mot_id, fill=main_overlay_motivation_color)
...
def update_clock():
    ...
    UI.after(1000, update_clock) # noqa
...
def update_color2():
    ...
    index = (index + 1) % len(txcolor)
    UI.after(16, update_color2) # noqa

mot_bbox = canvas.bbox(mot_id)
mot_height = mot_bbox[3] - mot_bbox[1] if mot_bbox else main_overlay_motivation_size
...
def update_spotify_text():
    ...
    UI.after(2000, update_spotify_text) # noqa
...
def update_color3():
    ...
    index = (index + 1) % len(txcolor)
    UI.after(16, update_color3) # noqa
...
def force_topmost2():
    UI.attributes("-topmost", True)
    UI.after(500, force_topmost2) # noqa
UI.after(100, force_topmost2) # noqa
